...
const INFURA_API_KEY = '45a27bf77c7d4f70a744a35738ec07b7';//節點
web3 = new Web3(`https://mainnet.infura.io/v3/${INFURA_API_KEY}`);//實體物件
...
mygasprice = web3.utils.toWei('12.0', 'Gwei'); //把 5 Gwei轉換成 wei(礦工費改成可調會更好)
// 查詢測試鏈Ｇoerli 的即時gas price可以連上網站 https://stats.goerli.net/ 查詢
console.log('gasprice:',mygasprice);

function queryBlocks(){//三種鏈
    web3.eth.getBlockNumber((err, results) => { //eth的官方函數
...
function queryBalance(event) {
    address = document.getElementById('walletaddress').value//取值
  web3.eth.getBalance(address, (err, balance) => {//address為引數
